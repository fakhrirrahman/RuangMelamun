                ...
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light rounded h-100 p-4">
                            <h6 class="mb-4">Data Kegiatan 2</h6>
                            <a href="{{route('kegiatan2.create')}}" type="button" class="btn btn-primary m-2">Tambah Kegiatan 2</a>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach($kegiatan2 as $booking2)
                                        <tr>
                                            <th scope="row">{{ $loop->iteration }}</th>
                                            <td>{{ $booking2->isi }}</td>
                                            <td><img src="{{ asset('foto_kegiatan2/' . $booking2->foto) }}" alt="foto_kegiatan2" width="100"></td>
                                            <td>
                                                <a href="{{ route('kegiatan2.edit', $booking2->id) }}" class="btn btn-primary">Edit</a> 
                                            </td>
                                            <td>
                                                <form action="{{ route('kegiatan2.destroy', $booking2->id) }}" method="POST" onsubmit="return confirm('Delete?')">
                                                    @csrf
                                                    @method('DELETE')
