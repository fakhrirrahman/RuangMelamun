                ...
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light rounded h-100 p-4">
                            <h6 class="mb-4">Data Kegiatan 1</h6>
                            <a href="{{route('kegiatan1.create')}}" type="button" class="btn btn-primary m-2">Tambah Kegiatan 1</a>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach($kegiatan1 as $booking1)
                                        <tr>
                                            <th scope="row">{{ $loop->iteration }}</th>
                                            <td>{{ $booking1->judul }}</td>
                                            <td>{{ $booking1->isi }}</td>
                                            <td>{{ $booking1->tanggal }}</td>
                                            <td><img src="{{ asset('foto_kegiatan1/' . $booking1->foto) }}" alt="foto_kegiatan1" width="100"></td>
                                           <td>
                                            <a href="{{route('kegiatan1.edit', $booking1->id)}}" type="button" class="btn btn-primary">Edit</a> 
                                        </td>
                                           <td>
                                                  <form action="{{ route('kegiatan1.destroy', $booking1->id) }}" method="POST" type="button" class="btn btn-danger p-0" onsubmit="return confirm('Delete?')">
                                                      @csrf
                                                      @method('DELETE')
