                ...
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light rounded h-100 p-4">
                            ...
                            <a href="{{route('slide.create')}}" type="button" class="btn btn-primary m-2">Tambah Slide</a>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach($slide as $book)
                                        <tr>
                                            <th scope="row">{{ $loop->iteration }}</th>
                                            <td>{{ $book->judul }}</td>
                                            <td>{{ $book->isi }}</td>
                                            <td>{{ $book->link }}</td>
                                            <td><img src="{{ asset('foto_slide/' . $book->foto) }}" alt="foto_buku" width="100"></td>

                                           <td><a href="{{route('slide.edit', $book->id)}}" type="button" class="btn btn-primary">Edit</a> </td>
                                           <td>
                                                  <form action="{{ route('slide.destroy', $book->id) }}" method="POST" type="button" class="btn btn-danger p-0" onsubmit="return confirm('Delete?')">
                                                      @csrf
                                                      @method('DELETE')
